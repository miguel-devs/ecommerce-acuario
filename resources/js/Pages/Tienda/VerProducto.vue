      ...
      <section class="w-full min-h-screen bg-sky-600 pt-20 pl-10 pr-10 pb-10">
        <div class="w-full bg-white rounded-md shadow-md pt-6">
            ...
            <!-- Image gallery -->
             ...
             <div class=" h-96 p-2 overflow-y-auto scroll scroll-track scroll-thumb">
                <div class="mt-3 bg-white rounded-md flex justify-center p-1 cursor-pointer shadow-xl border border-slate-100" v-for="(imageproduct,index) in product.image_products" :key="index">
                  <img  :src="`/imagenes/productos/${imageproduct.nombre}`" @click="imgPreview = imageproduct.nombre"/>
                </div>
              </div>
              <div class=" h-96 col-span-3 flex justify-center">
                <div class="lg:w-2/5 aspect-w-4 aspect-h-5 sm:rounded-lg sm:overflow-hidden lg:aspect-w-3 lg:aspect-h-4">
                <img v-if="imgPreview"
                  :src="`/imagenes/productos/${imgPreview}`"
                  :alt="product.nombre"
                  class="w-full h-full object-center object-cover"
                />
                <img v-else
                :src="`/imagenes/productos/${product.imagen}`"
                  :alt="product.nombre"
                  class="w-full h-full object-center object-cover"
                />
                </div>
            ...
            >
              ...
              <div class="mt-4 lg:mt-0 lg:row-span-3">
                ...
                <div v-else> 
                <button v-if="desabilitarButton" class="mt-10 w-full bg-sky-600 shadow-md rounded-md py-3
                    px-8 flex items-center justify-center text-base font-medium text-white  cursor-not-allowed" :disabled="desabilitarButton">
                  Agregar al carrito 
                </button>
              ...
              >
                ...
                <div class="mt-10">
                  ...
                  <div
                    class="mt-4 space-y-6 text-sm text-gray-600"
                    v-html="product.detalles"
                  ></div>
                </div>
              </div>
            </div>

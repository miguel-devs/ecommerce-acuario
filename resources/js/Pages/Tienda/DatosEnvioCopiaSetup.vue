...
const props =  defineProps({
	"errors":Object
      ...
      let paymentMensaje = ref(null); 
      let totalCarrito= ref(null);
      let totalPrecioCarrito= ref(null);
      
      const formDatosEnvio= useForm({
        nombre: null,
        correo: null,
        telefono: null,
        calle: null,
...
onMounted(async () => {
    ...
    if (localStorage.getItem("carrito")) {
     ...
     totalCarrito.value = JSON.parse(localStorage.getItem("carrito")).length;
    }
    calcularPrecioTotal();
     ...
     const processPayment = () => {
               ...
               stripe.createPaymentMethod(
                    'card', cardElement, {
                        billing_details: {
                            name: formDatosEnvio.nombre,
                            email: formDatosEnvio.correo,
                            address: {
                                line1: formDatosEnvio.calle +" " +formDatosEnvio.noInterior ,
                                state: formDatosEnvio.estado,
                                postal_code: formDatosEnvio.cp
                            }
                        }
                    }
                 ...
                 if(result.error){
                    ...
                    formDatosEnvio.payment_method_id = paymentMethod.id;
                    formDatosEnvio.total = totalPrecioCarrito;
                    formDatosEnvio.carrito = carrito;
                     
                    

                   formDatosEnvio.post("/realizar-orden", {
                    preserveScroll: true,
                    replace: true ,
                    preserveState: false,
                    onError: () => {
    ...
    const formattedCurrencyValue = (precio,cantidad) => {
    ...
    const calcularPrecioTotal = () => {
                 totalPrecioCarrito.value = null;
                
                 carrito.forEach((item, index) => {
                    totalPrecioCarrito.value += item.precio * item.cantidad;
                 });
                 totalPrecioCarrito.value = formattedTotal(totalPrecioCarrito);
    }
